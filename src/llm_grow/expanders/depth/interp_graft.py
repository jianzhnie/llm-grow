...
新层从"有意义"的初始化出发，收敛速度优于 OverlapSplit (DUS)。
即时精度约 80-90%（非严格 FP），CPT 需求量约为 OverlapSplit 的 50%。

原始论文: Yang et al., "LESA: Learnable LLM Layer Expansion with
    SVD-based Adaptation", arXiv:2502.13794, 2025.

Related:
    - ``IdentityGraft`` (identity_graft.py): 严格 FP 恒等块嫁接
    - ``OverlapSplit`` (overlap_split.py): 非 FP 层重叠拷贝
"""
