...
Related:
    - ``IdentityGraft`` (identity_graft.py): 仅深度的 FP 扩增
...
def _expand_depth(model: nn.Module, config: MultiAxisGrowConfig) -> nn.Module:
    ...
    sub_config = IdentityGraftConfig(
    ...
    return IdentityGraftExpander().expand(model, sub_config)
